...
class MaybeTest extends TestCase {
  ...
  public function testJustMapComposition() {
    ...
    $a = Maybe::of("baz")->map(function($x) use ($f, $g) {
      return $f($g($x));
    });
  ...
  public function testJustApIdentity() {
    ...
    $this->assertEquals($b->ap(Maybe::of(function($x) {
      return $x;
    })), $b);
  }
  ...
  public function testNothingGetOrElse() {
    $a = Maybe::fromNullable(null)->map(function($x) {
      return 'bar' . $x;
    })->getOrElse('foo');
  ...
  public function testMonadRightIdentity() {
    ...
    $this->assertEquals($m, $m->chain(function($x) {
      return Maybe::of($x);
    }));
  }
